...
int gcd_rec(int small, int large) {
  if (large < small) { return gcd_rec(large, small); }
  else {
    if (small == 0) { return large; }
    else { return gcd_rec(large % small, small); }
  }
}
...
struct poly {
  ...
  struct const_iterator {
    public:
      operator monomial () {
        return monomial(it->second, it->first);
      }
...
string str () const {
  ...
  for(auto it : this->coeffs) {
    ...
    out += to_string(abs(it.second)); out += it.first.str();
    first = false;
  }
  return out;
}
...
poly operator *(R c, const exponent &e) {
  return poly(c, e);
}
...
bool does_monomial_divide_poly(monomial m, poly p) {
  for(auto it : p) {
    optional<exponent> delta = it.exp - m.exp;
